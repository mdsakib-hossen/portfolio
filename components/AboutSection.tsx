        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="flex justify-center">
            <div className="relative">
              ...
              <div className="gradient-border relative w-56 h-56 md:w-72 md:h-72 rounded-2xl overflow-hidden">
                {photo ? (
                  <Image
                    src={photo}
                    alt={name}
          ...
          <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-gray-400 leading-relaxed mb-8">{about}</p>
        ...
        <div ref={ref} className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-16">
          {stats.map(({ value, suffix, label }, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              className="glass p-6 rounded-2xl text-center glow-purple">
              <div className="text-4xl font-bold gradient-text mb-2">
                {inView && <CountUp end={value} duration={2} suffix={suffix} />}
              </div>
              <p className="text-gray-400 text-sm">{label[language]}</p>
            </motion.div>
          ))}
        </div>
      </motion.div>
