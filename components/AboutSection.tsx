...
export default function AboutSection() {
  ...
  const about = profileData?.about_en || "Computer Science & Technology student at Bangladesh Polytechnic Institute, Rajshahi. Passionate about Competitive Programming, Software Development, and AI/ML.";
    ...
    <section id="about" className="section-padding w-full">
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
        <div className="text-center mb-8 md:mb-16">
          <p className="text-cyan-400 font-mono text-sm tracking-widest mb-2">// GET TO KNOW ME</p>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="flex justify-center">
            <div className="relative">
              <div className="absolute inset-0 bg-gradient-to-br from-cyan-600 to-sky-600 rounded-2xl blur-xl opacity-30 scale-105" />
              <div className="gradient-border relative w-44 h-44 sm:w-56 sm:h-56 md:w-72 md:h-72 rounded-2xl overflow-hidden">
                {photo ? (
                  <Image src={photo} alt={name} fill className="object-cover" />
                ) : (
                  <div className="w-full h-full bg-gradient-to-br from-cyan-600/30 to-sky-600/30 flex items-center justify-center text-6xl">
                    👤
                  </div>
                )}
              </div>
              <div className="absolute -bottom-4 -right-4 glass px-2 sm:px-3 py-1.5 rounded-full flex items-center gap-2 max-w-[90%]">
                ...
                <span className="text-xs text-green-400 truncate">Open to Collaboration</span>
              </div>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="mt-8 lg:mt-0">
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-4">Hi, I&apos;m Sakib 👋</h3>
            <p className="text-gray-400 leading-relaxed mb-6 sm:mb-8 text-sm sm:text-base">{about}</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                { label: "Institute", value: "Bangladesh Polytechnic Institute" },
                { label: "Department", value: "CST" },
        ...
        <div ref={ref} className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mt-12 sm:mt-16">
          {stats.map(({ value, suffix, label }, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              className="glass p-4 sm:p-6 rounded-2xl text-center glow-cyan">
              <div className="text-2xl sm:text-4xl font-bold gradient-text mb-2">
                {inView && <CountUp end={value} duration={2} suffix={suffix} />}
              </div>
              <p className="text-gray-400 text-xs sm:text-sm">{label["en"]}</p>
            </motion.div>
          ))}
        </div>
      </motion.div>
